...
public class PointManagerFlight : PointManager
{
    ...
    public PointManagerFlight( DomainManager domain, RunOptions configOptions, LGOptionsPointsFlights configSubOptions, Random rng ) : base(
    ...
    {
        AdvectedPointContrail point = (AdvectedPointContrail)base.NextPoint(x, y, pressure);
        ...
        foreach (string flightLabel in FlightTable.Keys)
        {
            ...
            foreach (FlightSegment flightSegment in flight.SegmentList)
            {
                ...
                foreach (FlightSegment.Waypoint seed in seedVector)
                {
                    ...
                    AdvectedPointConnected newPoint = (AdvectedPointConnected)NextPoint(seed.Lon, seed.Lat, seed.Pressure);
                    if (ContrailSimulation)
                    {
                        ((AdvectedPointContrail)newPoint).InitiateContrail(flight.Equipment);
                    }
    ...
    protected class Flight
    {
        ...
        public AdvectedPointConnected? LastPoint { get; protected set; } = null;
        ...
        public void UpdateLast(AdvectedPointConnected newPoint)
        {
            LastPoint = newPoint;
        }

        public void ClearPoint(AdvectedPointConnected caller)
        {
            if (caller == LastPoint)
            {
                LastPoint = null;
