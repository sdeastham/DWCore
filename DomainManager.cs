...
public class DomainManager
{
    ...
    public double[,,] BruntVaisalaXYP // s-1
    { get; protected set; }
    ...
    public double[,,] BoxEdgeAltitudeXYPe // m
    { get; protected set; }
        ...
        bool fixedPressures = false)
    {
        ...
        double earthCircumference = 2.0 * Math.PI * Physics.EarthRadius;
        double edgeLength;

        // South boundary
        edgeLength = (DX/360.0) * earthCircumference * Math.Cos(Physics.Deg2Rad*latEdge[0]);
        for (int i=0;i<NX;i++)
        {
            BoundaryLengths[i] = edgeLength;
        }
        // North boundary
        edgeLength = (DX/360.0) * earthCircumference * Math.Cos(Physics.Deg2Rad*latEdge[yPosts-1]);
        for (int i=0;i<NX;i++)
        {
    ...
    private void SetCellAreas()
    {
        ...
        {
            yRad[i] = YMesh[i] * Physics.Deg2Rad;
        }

        // Total surface area in each meridional band (assuming regular spacing)
        double bandArea = Physics.EarthRadius * Physics.EarthRadius * DX * Physics.Deg2Rad;
    ...
    public (double, double, double) VelocityFromFixedSpaceArray( double x, double y, double pressure, bool noConvert=false)
    {
        ...
        dxdt = Physics.Rad2Deg * dxdt / (Physics.EarthRadius * Math.Cos(Physics.Deg2Rad*y));
        dydt = Physics.Rad2Deg * dydt / Physics.EarthRadius;
        return (dxdt, dydt, dpdt);
    }
    ...
    public (double[], double[], double[], double) SeedBoundary(double kgPerPoint, double dt, Random RNG, double massSurplus = 0.0)
    {
        ...
        for (int i=0; i<nFaces; i++)
        {
            ...
            massFlux = dt * vNorm * faceArea[i] / Physics.G0;
    ...
    public (double[], double[], double[], double) SeedPressureBoundaries(double kgPerPoint, double dt, Random RNG, double massSurplus = 0.0)
    {
        ...
        for (int i=0; i<NX; i++)
        {
            for (int j=0; j<NY; j++)
            {
                ...
                vNorm = -1.0;
                k = BaseLevel[j,i];
                massFlux =  dt * (vNorm * PressureVelocityXYP[k,j,i]) * CellArea[j,i] / Physics.G0;
